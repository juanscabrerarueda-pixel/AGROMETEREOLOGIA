...
export default function App() {
    if (!FEATURE_AGROMETEO) {
        return (_jsx("main", { className: "wrap", children: _jsxs("section", { className: "card", children: [_jsx("h1", { children: "Panel agrometeorolA3gico" }), _jsx("p", { children: "Activa la variable VITE_FEATURE_AGROMETEO para visualizar este tablero." })] }) }));
    }
...
function buildChartNarrative(summary, metric, rangeLabel, series, trendInfo) {
    ...
    if (metric === 'accumulated') {
        parts.push(`Entre ${rangeLabel} se acumularon ${formatNumber(summary.totalRain)} mm distribuidos en ${summary.count} dAas con datos.`);
        if (summary.maxValueDate) {
            parts.push(`El dAa mAs lluvioso fue ${formatDisplayDate(summary.maxValueDate)}, cuando se registraron ${formatNumber(summary.maxValue)} mm en 24 horas.`);
        }
    }
    else {
        parts.push(`Analizamos ${summary.count} dAas de intensidades entre ${rangeLabel}.`);
        if (summary.maxValueDate) {
            parts.push(`La rAfaga mAxima ocurriA3 el ${formatDisplayDate(summary.maxValueDate)} y alcanzA3 ${formatNumber(summary.maxValue)} mm/h.`);
        }
    }
