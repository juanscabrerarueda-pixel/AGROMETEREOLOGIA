...
export function insightsFromSeries(series: Series, thresholds: Thresholds): Insight[] {
  ...
  if (rootMoist != null) {
    if (rootMoist < 0.18) {
      ...
      insights.push({
        ...
        kind: 'advice',
        text: `Suelo muy h�medo (${(rootMoist * 100).toFixed(
          0
        )}%). Evita labores pesadas para no compactar ni dañar cultivos.`,
      });
    }
  }
  ...
  if (evapDemand != null && evapDemand > 5) {
    insights.push({
      id: 'et0-demand',
      kind: 'advice',
      text: `La ET� alcanz� ${evapDemand.toFixed(
        1
      )} mm en las �ltimas 24 h. Refuerza hidrataci�n animal o riego.`,
    });
  }
