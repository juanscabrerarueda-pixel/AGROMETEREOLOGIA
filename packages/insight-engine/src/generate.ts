...
export function insightsFromSeries(series: Series, thresholds: Thresholds): Insight[] {
  ...
  if (daily.totalDays > 0) {
    ...
    const averagesSentence = avgRainyDay
      ...
      ? `Cuando llovió, promedió ${formatNumber(avgRainyDay)} mm por día${
    ...
    const drySentence = daily.longestDry
      ...
      ? `La racha seca más larga ${isFuture ? 'proyectada sería' : 'duró'} ${

          daily.longestDry.length

        } días entre ${formatDate(daily.longestDry.from)} y ${formatDate(daily.longestDry.to)}.`
  ...
  if (peaks.length) {
    ...
    insights.push({
      ...
      )}. ${distributionSentence} Programa labores críticas fuera de esas ventanas para evitar daños por escorrentía.`,
